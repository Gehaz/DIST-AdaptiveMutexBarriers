...
As a corollary we get that each $p \in W$ executes exactly $t$ critical events along $D$ in the execution $J D$. Denote $\overline{W} = Act(J) \setminus W$. 

Since $J$ is a regular execution, by Lemma \ref{lem: remove-invisibale-processes} with $'E' \leftarrow J$ and $'Y' \leftarrow \overline{W}$, we have an execution $N = J^{\overline{W}}$ such that the following holds:
\begin{enumerate}
