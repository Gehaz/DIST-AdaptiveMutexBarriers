...
\end{inparaenum}
\\ Each variable $v$ is local to at most one process. Moreover, if $q = writer(v,J)$ such that $q \neq owner(v)$, since $J$ is a regular execution, by IN4 it follows that $owner(v) \notin Z' \subseteq Act(J)$, and therefore at most one new edge is introduced by rules a) and b).
Assume there is a process $p \neq q \in Z'$ which writes to $v$ in $\beta_q'$ such that $q \neq owner(v)$ and $writer(v,J) \neq q$. The first such write $e$ is critical. Notice that $e \neq e_q$, since by the definition of $Z'$ in contains no two process such that $e_p$ and $e_q$ writes to the same variable. Therefore, at most one new edge is introduced by rule c). Altogether, the average degree in $\mathcal{G}$ is at most 4. By Theorem \ref{th: turan}, there exists an independent set $P_k \subseteq Z'$ in $\mathcal{G}$ such that:
...
\begin{enumerate}[(1)]
	...
	\item $P_k$ satisfies condition (2) with the prefixes $\beta_p$. We extended each $\beta_p$ in a way such that each process have one more critical write. Intuitively, since each such write is to a different variable, and the independent set assure that the owner and writer of this variable is not part of $P_k$, the condition will hold with the new prefixes $\beta_p'$ as well. Formally, let $e \in \beta_p'$ be a critical write in $J \beta_p'$ by some $p \in P_k$ to some variable $v$. We consider two cases:
