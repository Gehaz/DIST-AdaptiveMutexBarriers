...
First notice that $P_0=Act(J)$ satisfies all the above conditions with $\beta_p=\langle \rangle$ for any $p \in Act(J)$. Assume we already constructed $P_{k-1}$ satisfying the conditions of Lemma \ref{lemma-new}. We denote $n=|P_{k-1}$, and $\beta_p$ to be the prefix as promised by Lemma \ref{lemma-new} for any $p \in P_{k-1}$, throughout the rest of this section, in which we define the construction of the write phase, and prove Lemma \ref{lemma-new}.
...
%It follows that $P'$ satisfies condition (2) of Lemma \ref{lemma-new}, since we extended $\beta_p$ with non critical events only, and $P' \subseteq P_{k-1}$.
...
By the pigeonhole principle, there exists a variable $u \in V_{next}$ and a set $P_k \subseteq Z$ of size $|P_k| \geq \sqrt{|Z|}$, such that $e_p$ is a critical commit write to $u$ for any $p \in P_k$. We now prove that $P_k$ satisfies all the conditions of Lemma \ref{lemma-new}, where for each $p \in P_k$ we choose $\beta_p' = \beta_p \gamma_p e_p$ to be the prefix of $\alpha_p$.
\begin{enumerate}[(1)]
	...
	\item $P_k$ satisfies condition (2) with the prefixes $\beta_p$. Intuitively, since we extended each $\beta_p$ in a way such that all processes have one more critical write to the same variable $v$, the condition will hold with the new prefixes $\beta_p'$ as well. Formally, let $e \in \beta_p'$ be a critical write in $J \beta_p'$ by some $p \in P_k$ to some variable $v$. We consider two cases:
	
	Assume $e \in \beta_p$. Since $P_k \subseteq P_{k-1}$ conditions (a) and (b) follows immediately. In addition, if there is a process $q \in P_k$ which writes to $v$ in $\beta_q'$, then the first such write in $J \beta_q'$ is critical (by conditions (a) and (b)). If this write is in $\beta_q$, then by induction, all processes in $P_k \subseteq P_{k-1}$ has a write to $v$ in $\beta_q$, and thus in $\beta_q'$. Otherwise, this write must be $e_q$, since $\gamma_q$ contains no critical writes. In case I, it follows that $v=u$ and $p$ writes to $v$ before $e_p$ in $\beta_p'$, contradicting the fact that $e_p$ is critical. In case II, we get that 
	
	Otherwise, $e=e_p$ and $v=u$. By the construction, any $q \in P_k$ have a critical write to $v$ in $J \beta_q'$ and condition (c) holds. Since $\beta_q'$ is a solo run by $q$ and there is a critical write to $v$ in $\beta_q'$, we can conclude that $owner(v) \neq q$ and $writer(v,J) \neq q$. Altogether we get that $owner(v) \notin P_k$ and $writer(v,J) \notin P_k$, and conditions (a) and (b) holds.
	
	\item $|P_k| \geq \sqrt{|Z|} \geq \sqrt{|P_{k-1}|/2} \geq \sqrt{|P_{k-1}|}/10$.
...
\begin{inparaenum}[\itshape a\upshape)]
	...
	\item $writer(v,J) = q$; or
	\item $q$ writes to $v$ in $\beta_q'$.
\end{inparaenum}
\\  Fix a process $p \in Z'$ such that $e_p = write(v)$. Each variable $v$ is local to at most one process. Moreover, if $q = writer(v,J)$ such that $q \neq owner(v)$, since $J$ is a regular execution, by IN4 it follows that $owner(v) \notin Z' \subseteq Act(J)$, and therefore at most one new edge is introduced by rules a) and b).
Assume there is a process $p \neq q \in Z'$ which writes to $v$ in $\beta_q'$ such that $q \neq owner(v)$ and $q \neq writer(v,J)$. Thus, the first such write $e$ in $\beta_q'$ is critical. Notice that $e \neq e_q$, since by the definition of $Z'$ it contains no two process such that $e_p$ and $e_q$ writes to the same variable. Therefore, at most one new edge is introduced by rule c). Altogether, the average degree in $\mathcal{G}$ is at most 4. By Theorem \ref{th: turan}, there exists an independent set $P_k \subseteq Z'$ in $\mathcal{G}$ such that:

$$ |P_k| \geq \frac{|Z'|}{5} \geq \frac{\sqrt{|Z|}}{5}$$
...
\begin{claim-subsection}
	$P_k$ satisfies condition (2) of Lemma \ref{lemma-new}.
\end{claim-subsection}
 
 \begin{proof}
 	Notice that $P_k$ satisfies condition (2) with the prefixes $\beta_p$, since $P_{k-1}$ did. Each $\beta_p$ was extended such that $p$ have one more critical write. In case I all processes have one more critical write to the same variable $u$. In case II all new critical writes are to different variables, and the independent set assure that the owner and writer of those variable are not part of $P_k$. This gives an intuition of why condition (2) holds with the new prefixes $\beta_p'$ as well. Formally, let $e \in \beta_p'$ be a critical write in $J \beta_p'$ by some $p \in P_k$ to some variable $v$. We consider two cases:
 	
 	Assume $e \in \beta_p$. Since conditions (a) and (b) holds for $e$ with $P_{k-1} \supseteq P_k$, it follows immediately they both hold with $P_k$. In addition, if there exists a process $q \in P_k$ which writes to $v$ in $\beta_q'$, then the first such write $e'$ is critical in $J \beta_q'$ (by conditions (a) and (b)). If $e' \in \beta_q$ then both $p,q$ commit a critical write to $v$ in $\beta_p,\beta_q$ (respectively). By condition (c) all processes in $P_k \subseteq P_{k-1}$ has a write to $v$ in $\beta_q$, and thus in $\beta_q'$, and condition (c) holds.
 	\\ Since $\gamma_q$ contains no critical events, the only other option is $e = e_p$. We now prove this can not occur. Assume towards a contradiction $e' = e_q$. In case I, since all new critical events are to the same variable, it follows that $v=u$. Therefore, $p$ writes to $v$ twice in $\beta_q'$ which is a solo run, contradicting the fact that the last such write $e_p$ is critical. In case II, we get that $q$ writes in $e_q$ to a variable that was written to in $\beta_p'$, and thus there is an edge $\{p,q\}$ in $\mathcal{G}$, contradicting the fact that $p,q \in P_k$, an independent set.
...
\begin{enumerate}[(1)]
	...
	\item Notice that $P_k$ satisfies condition (2) with the prefixes $\beta_p$, since $P_{k-1}$ did. We extended each $\beta_p$ in a way such that each process have one more critical write. In case I we extended each $\beta_p$ in a way such that all processes have one more critical write to the same variable $v$. In case II all new critical writes are to different variables, and the independent set assure that the owner and writer of those variable are not part of $P_k$. Thus, intuitively the condition will hold with the new prefixes $\beta_p'$ as well. Formally, let $e \in \beta_p'$ be a critical write in $J \beta_p'$ by some $p \in P_k$ to some variable $v$. We consider two cases:
