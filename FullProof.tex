...
\begin{lemma} \label{write-phase-lemma}
	At each step during the write phase we have an execution $J_k$ satisfying the following conditions:
	\begin{enumerate}[(1)]
		\item $J_k$ is a semi-regular ordered execution;
		\item Each $p \in Act(J_k)$ executed $\delta+k$ critical events in $J_k$;
...
First notice that $J_0$ satisfies all the conditions in Lemma \ref{write-phase-lemma}, since $J_0$ is a regular execution (any regular execution is also ordered, as one of (a) or (b) holds for any variable). Assume we already constructed $J_{k-1}$ satisfying the conditions of Lemma \ref{write-phase-lemma}. We denote $J=J_{k-1},\ n=|Act(J_{k-1})|$ throughout the rest of this section, in which we define the construction of the write phase, and prove Lemma \ref{write-phase-lemma}.
