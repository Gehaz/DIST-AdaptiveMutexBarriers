...
\begin{lemma} \label{lemma:read-new}
	...
	\begin{enumerate} [(1)]
		...
		\item Let $e \in \beta_p$ be a critical read in $H_p \beta_p$ by some process $p \in P_k$ to some variable $v$. Then:
		\begin{enumerate}[(a)]
			\item $owner(v) \notin P_k$;
			\item $writer(v,H) \notin P_k$;
		\end{enumerate}
	\item $|P_k| \geq |P_{k-1}|/10$.
	\end{enumerate}
\end{lemma}
...
Before going into the details of the construction, we first prove a claim which states that indeed, giving a set of processes $P'$ such that their $\alpha_p$ satisfies condition (2) of Lemma \ref{lemma:read-new}, then erasing processes not in $P'$ from $H$ and afterwards extend it using $\alpha_p$ of processes in $P'$ in any order results an execution. Giving a set $P'$ we denote by $\alpha(P')$ the execution fragment in which we concatenate the fragments $\alpha_p$ of processes in $P'$ in some order.

\begin{claim-subsection}
	Let $P' \subseteq Act(H)$ be a set such that condition (2) of Lemma \ref{lemma:read-new} holds for $P'$ with $\alpha_p$. Then $H^{-Act(H) \setminus P'} \alpha(P')$ is an execution.
...
$$|P_k| \geq \frac{|Z|}{3} \geq \frac{|P_{k-1}|/2}{3} = \dfrac{|P_{k-1}|}{6}$$
